...
parser.add_argument('--config_file', type=str, default='snn_conv_1_gesture_max.yaml',
...
class GestureDataset(Dataset):
    ...
    def get_spike_train(self, event, length):
        ...
        spike_train /= spike_train.amax(dim=(0, 1, 2))  # max
...
if __name__ == "__main__":
    ...
    if args.train == True:
        if args.load is True:
            print('load data')
            train_data = torch.load('./data/DvsGesture/train_data_max.pt')
        else:
            print('process data')
            train_data = GestureDataset(root='./data/DvsGesture', train=True, length=length)
            torch.save(train_data, './data/DvsGesture/train_data_max.pt')
        ...
        if args.load is True:
            print('load data')
            test_data = torch.load('./data/DvsGesture/test_data_max.pt')
        else:
            print('process data')
            test_data = GestureDataset(root='./data/DvsGesture', train=False, length=length)
            torch.save(test_data, './data/DvsGesture/test_data_max.pt')
